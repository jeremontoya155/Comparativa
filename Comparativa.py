...
class Aplicacion:
    def __init__(self, root):
        ...
        self.root = root
        self.root.geometry("680x550")  # Anchura x Altura
        self.root.title("Comparador")
        self.root.config(bg="black")

        # Título
        self.titulo_label = tk.Label(root, text="DATOS PARA COMPARATIVA", bg="black", fg="white", font=("Arial", 20, "bold"))
    ...
    def cargar_archivo(self):
        # Abrir el diálogo para seleccionar el archivo CSV o XLSX
        ...
        if ruta_archivo:
            ...
            if ruta_archivo.endswith('.csv'):
                ...
                self.base_de_datos_df = pd.read_excel(ruta_archivo)
                self.base_de_datos_TXT = pd.read_excel(ruta_archivo)
            else:
                # Si la extensión no es compatible, mostrar un mensaje de error
                tk.messagebox.showerror("Error", "Formato de archivo no compatible. Por favor, selecciona un archivo CSV o XLSX.")
                return
            
            print(self.base_de_datos_df)  # Imprimir el DataFrame cargado
            
    def reordenar_y_renombrar(self, titulos_csv=True):
        ...
        if self.base_de_datos_df is not None:
            ...
            if titulos_csv:
                columnas = {
                    ...
                    'Comprar': 'ComprarQ',
                    'Máximo 3 meses': 'Máximo 3 meses',
                    'Vtas 01mes Atras Cerrado': 'Vtas 01mes Atras Cerrado',
                    'Vtas 02mes Atras Cerrado': 'Vtas 02mes Atras Cerrado',
                    'Vtas 03mes Atras Cerrado': 'Vtas 03mes Atras Cerrado',
                    'Stock Actual C.D.': 'Stock Actual C.D.',
                    'Stock Sucursales': 'Stock Sucursales',
                    'Surtido Total': 'Surtido Total',
                    'Precio': 'PVP'
                }
            else:
                columnas = {
                    ...
                    'Comprar': 'ComprarQ',
                    'Precio': 'PVP',
                    'MÃ¡ximo 3 meses': 'Máximo 3 meses',
                    'Vtas 01mes Atras Cerrado': 'Vtas 01mes Atras Cerrado',
                    'Vtas 02mes Atras Cerrado': 'Vtas 02mes Atras Cerrado',
                    'Vtas 03mes Atras Cerrado': 'Vtas 03mes Atras Cerrado',
                    'Stock Actual C.D.': 'Stock Actual C.D.',
                    ...
                    'Cod Alfabeta': 'Cod Alfabeta',
                    'Cod. Kairos': 'Cod. Kairos',
                    'DÃ­as de Stock': 'Días de Stock'
                }
                
            self.base_de_datos_df = self.base_de_datos_df.rename(columns=columnas)
    ...
    def exportar_a_xlsx(self):
        if self.cofar_df is not None:
            ...
            if self.base_de_datos_df is not None and self.proveedor_df is not None:
                ...
                self.exportacion_df["DDS"] = pd.to_numeric(self.exportacion_df["DDS"], errors='coerce').fillna(0)
                    # Realizar la división
                    # Convertir la columna "COFARSUR" a tipo numérico y rellenar NaN con 0
                    # Realizar la división
                self.exportacion_df["DDS"] = self.exportacion_df["DDS"] - (self.exportacion_df["DDS"]*0.08) 
                ...
                # Fusionar los DataFrames en base a la columna "C.Barra"
                    ...
                    self.exportacion_df["COFARSUR"] = pd.to_numeric(self.exportacion_df["COFARSUR"], errors='coerce').fillna(0)
